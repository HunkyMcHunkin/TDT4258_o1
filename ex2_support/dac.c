...
{
	//holds the count of number of while iterations
	int count = 1;
	...
	//Number of counts per periode
	int countperiod = samplesPerSecond / frequency;

	//the CMU clock frequency
	int clockFrequency = 14000000; // ALTERNATIV clockFrequency = 140000 MhZ SLIK AT VI KAN DROPPE 1000 ETTERPÅ
	
	// the average clock periods needed to compleat one while iteration
	int clockPeriodsWhileLoop = 6;
	...
	length = length* (clockFrequency / (1000*clockPeriodsWhileLoop)); //( Må JEG FORKLARE AT 1000 KOMMER FRA TRANSFORMASJONEN MELLOM SEKUNDER TIL MILLI SEKUNDER?)
...
void makeSound_sawtooth(int frequency, int length)
{
	...
	//the CMU clock frequency in milli hertz
	int clockFrequency = 14000;
	
	// the average clock periods needed to compleat one while iteration
	int clockPeriodsWhileLoop = 10;
	
	//turn length into length into corresponding while itterations. length = {determed length in second} / {average while loop time}
	length = length * (clockFrequency / clockPeriodsWhileLoop); 
	...
	while (count < length) {
		...
		if (count % countperiod == 0) {
			//if so reset voltage to start value
			dacvolt = 0;
		}
		//Write the voltage to the DAC 
		*DAC0_CH0DATA = dacvolt;
...
void makeSound_triangle(int frequency, int length)
{
	...
	// the average clock periods needed to compleat one while iteration
	int clockPeriodsWhileLoop = 11;
	
	//turn length into length in ms into corresponding while itterations. length = {determed length in second} / {average while loop time}
	length = length * (clockFrequency / clockPeriodsWhileLoop);
	...
	while (count < length) {
		...
		*DAC0_CH1DATA = dacVolt;
		...
		if ((count % dacUpTime) == 0) {  
			...
			//checks if value have gone outside range, and cut the triangle signal if it have (THE VOLTAGE WILL GO OUTSIDE THE RANGE FOR LOW FREQUANCIES, MAKING THE SOUND UNBARABLE LOUD)
			if (dacVolt>100){   		
			      dacVolt=100;
			} else if (dacVolt < 0) {
				dacVolt = 0;
